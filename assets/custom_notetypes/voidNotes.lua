...
function onCreate()
		...
		for i = 0, getProperty('unspawnNotes.length')-1 do
			...
			if not getPropertyFromGroup('unspawnNotes', i, 'mustPress') and (dadName == "Void" or getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'voidNotes') then
				setPropertyFromGroup('unspawnNotes', i, 'texture', 'luacharacters/notes/voidNotes'); --Change texture Use this to change note texture but not necessary
				...
				if getPropertyFromGroup('unspawnNotes', i, 'isSustainNote') then
					...
					local scl = getPropertyFromGroup('unspawnNotes', i, 'scale.y')*44/119;
