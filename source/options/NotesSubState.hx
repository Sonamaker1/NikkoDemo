...
class NotesSubState extends MusicBeatSubstate
{
	...
	function resetValue(selected:Int, type:Int) {
		...
		var clValue:Int = ClientPrefs.arrowHSVDefault[selected][type];
		ClientPrefs.arrowHSV[selected][type] = clValue;
		switch(type) {
			case 0: shaderArray[selected].hue = clValue / 360;
			case 1: shaderArray[selected].saturation = clValue / 100;
			case 2: shaderArray[selected].brightness = clValue / 100;
		}

		var item = grpNumbers.members[(selected * 3) + type];
		item.changeText( Std.string(clValue) );
