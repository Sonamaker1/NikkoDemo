...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		//var bgYellow:FlxSprite = new FlxSprite(-200, 0).makeGraphic(Std.int(FlxG.width*1.4), 486, 0xFFb6ff32); //0xFFF9CF51);
		//var bgBLK:FlxSprite    = new FlxSprite(-200, -156).makeGraphic(Std.int(FlxG.width*1.4), 46, 0xFF000000); //0xFFF9CF51);
		//var bgPink:FlxSprite   = new FlxSprite(-200, -156).makeGraphic(Std.int(FlxG.width*1.4), 26, 0xFFc34bff); //0xFFF9CF51);
		
		var menuRight:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('menu01'));
		menuRight.alpha = 1;
		menuRight.scrollFactor.set();
		menuRight.setGraphicSize(Std.int(FlxG.width),Std.int(FlxG.height)); //flipped to scale
		menuRight.screenCenter(X);
		menuRight.screenCenter(Y);
		//menuRight.y-=FlxG.height-100;
		...
		//bgYellow.antialiasing = ClientPrefs.globalAntialiasing;
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			weekThing.y += ((weekThing.height + 20) * (i+1));
			weekThing.targetY = i;
			weekThing.targetX = 30 + (weekThing.width*1.2) * (i);
			weekThing.targetYOffset  = 400;
			weekThing.targetYSpacing = 180;
			grpWeekText.add(weekThing);

			//-- weekThing.screenCenter(X);
			weekThing.forceX = 30 + (weekThing.width*1.2) * (i);
		...
		for (char in 0...3)
		{
			...
			weekCharacterThing.y += -120 + (char * 90);
			weekCharacterThing.scale.set(0.65,0.65);
		...
		sprDifficulty = new FlxSprite(0, leftArrow.y-120);
		...
		txtTracklist.color = 0xFFb6ff32;//0xFFe55777;
		add(txtTracklist);
		// add(rankText);
		add(scoreText);
		add(txtWeekTitle);
	...
	override function update(elapsed:Float)
	{
		...
		{
			var incWeekKey = controls.UI_RIGHT_P;
			var decWeekKey = controls.UI_LEFT_P;
	...
	function changeDifficulty(change:Int = 0):Void
	{
		...
		if(newImagePath != lastImagePath)
		{
			...
			tweenDifficulty = FlxTween.tween(sprDifficulty, {y: leftArrow.y - 60, alpha: 1}, 0.07, {onComplete: function(twn:FlxTween)
			{
	...
	function changeWeek(change:Int = 0):Void
	{
		...
		for (item in grpWeekText.members)
		{
			...
			var moreBS:Float = 30 + (item.width*1.2) * (item.targetY+1);
