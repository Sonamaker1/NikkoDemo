...
class FunkinLua {
	...
	public function new(script:String) {
		...
		set('validScore', PlayState.SONG.validScore);
		...
		set('playerIsDead', PlayState.instance.isDead);
		...
		Lua_helper.add_callback(lua, "quickCheckUnlocked", function(song:String, conditions:String) {
			var remainder:String = '';
			var splitArr:Array<String> = conditions.split('|');
			if(splitArr.length>0){
				for(prereq in 0...splitArr.length)
				{
					//var boolExists = Highscore.scoreExists(songs[i].prereqCompletions[prereq],0);
					var songP:String = splitArr[prereq];
					var scoreP:Int = Highscore.getScore(songP,0);
					if(scoreP <= 0)
					{
						remainder = songP;
						break;
		...
		Lua_helper.add_callback(lua, "fullCheckUnlocked", function(song:String, conditions:String) {
			var remainder:String = '';
			var splitArr:Array<String> = conditions.split('|');
			if(splitArr.length>0){
				var incr:Int = 0;
		...
		Lua_helper.add_callback(lua, "setPropertyFromGroup", function(obj:String, index:Int, variable:Dynamic, value:Dynamic) {
			...
			if(leArray!=null && leArray.length > index){
				leArray = leArray[index];
			}
			if(leArray != null) {
				if(Type.typeof(variable) == ValueType.TInt) {
		...
		Lua_helper.add_callback(lua, "removeFromGroup", function(obj:String, index:Int, dontDestroy:Bool = false) {
			if(Std.isOfType(Reflect.getProperty(getInstance(), obj), FlxTypedGroup)) {
				var sex:Dynamic = Reflect.getProperty(getInstance(), obj);
				if(sex!=null && sex.members!=null && sex.members.length > index){
					sex =  sex.members[index];
				}
				else{
					return;
				}
		...
		Lua_helper.add_callback(lua, "callMethod", function(method:String, args:Array<Dynamic>) {
			var splitShits:Array<String> = method.split('.');
			if (splitShits.length > 1) {
				var coverMeInPiss:Dynamic = null;
				if (PlayState.instance.modchartSprites.exists(splitShits[0]) )
				{
					coverMeInPiss = Reflect.getProperty(PlayState.instance.modchartSprites.get(splitShits[0]), splitShits[0]);
				} 
				else 
				{
					coverMeInPiss = Reflect.getProperty(PlayState.instance, splitShits[0]);
				}

				for (i in 1...splitShits.length-1) 
				{
					coverMeInPiss = Reflect.getProperty(coverMeInPiss, splitShits[i]);
				}
				return Reflect.callMethod(coverMeInPiss, Reflect.field(coverMeInPiss, splitShits[splitShits.length-1]), args);
			}
			return Reflect.callMethod(PlayState.instance, Reflect.field(PlayState.instance, method), args);
		});
		...
		Lua_helper.add_callback(lua, "startDialogue", function(dialogueFile:String, music:String = null, bgcolorStr:String = "0xFFFFFFFF") {
			/* 
			//this stinks I want to be able to use the same dialog file for the unlock 
			var path:String = Paths.modsJson(Paths.formatToSongPath(PlayState.SONG.song) + '/' + dialogueFile);
			if(!FileSystem.exists(path)) {
				path = Paths.json(Paths.formatToSongPath(PlayState.SONG.song) + '/' + dialogueFile);
			}
			*/
			var path = dialogueFile + ".json";
			if(FileSystem.exists(Paths.modFolders(path))) {
				path = Paths.modFolders(path);
			} else {
				path = Paths.getPreloadPath(path);
			} 
			...
			if(FileSystem.exists(path)) {
				...
				if(shit.dialogue.length > 0) {
					PlayState.instance.startDialogue(shit, music, bgcolorStr);
